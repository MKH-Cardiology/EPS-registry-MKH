...
el.style.boxShadow = ‘’;
const tip = el.parentNode.querySelector(’.val-tip’);
if(tip) tip.remove();
}
...
// ============================================================
// ATTACH ALL NUMBER RANGE VALIDATIONS
// ============================================================
...
if(lvefEl) {
lvefEl.addEventListener(‘blur’, function() {
if(!this.value) { clearValidation(this); return; }
const v = parseFloat(this.value);
if(v < 5 || v > 85) {
showError(this, ‘Expected 5\u201385%’);
} else if(v < 10 || v > 80) {
showWarning(this, `LVEF ${v}% \u2014 unusual value, please confirm`);
} else {
clearValidation(this);
}
});
...
if(adm && this.value && this.value < adm) {
showError(this, ‘Discharge date cannot be before admission date’);
} else {
clearValidation(this);
}
});
...
const adm = document.getElementById(‘req_admDate’)?.value;
if(adm && this.value && this.value < adm) {
showError(this, ‘Procedure date cannot be before admission date’);
} else {
clearValidation(this);
}
});
...
showWarning(this, ‘Finish time should be after start time’);
} else {
clearValidation(this);
}
});

// 5. Civil ID century digit validation
document.getElementById(‘civilId’)?.addEventListener(‘blur’, function() {
const v = this.value;
if(v.length === 12 && /^\d{12}$/.test(v)) {
const c = parseInt(v[0]);
if(c !== 2 && c !== 3) {
showError(this, ‘Civil ID must start with 2 (born 1900s) or 3 (born 2000s)’);
} else {
clearValidation(this);
}
}
...
// 6. YOB and Estimated Age consistency
...
showWarning(document.getElementById(‘estAge’), `Age (${age}) doesn't match YOB ${yob} \u2014 expected ~${calcAge}`);
} else {
clearValidation(document.getElementById(‘estAge’));
}
}
}
...
// 7. LVEF <= 35% auto-suggest CRT/ICD indications
const lvefInput = document.querySelector(’[name=“lvef”]’);
lvefInput?.addEventListener(‘blur’, function() {
const v = parseFloat(this.value);
if(!isNaN(v) && v <= 35) {
const icmChecked  = document.querySelector(’[name=“crtInd_icmEF”]’);
const nicmChecked = document.querySelector(’[name=“crtInd_nicmEF”]’);
if(icmChecked && !icmChecked.checked && !nicmChecked?.checked) {
let banner = document.getElementById(‘lvef_suggest’);
if(!banner) {
...
banner.innerHTML = ‘\ud83d\udca1 LVEF \u2264 35% detected \u2014 consider checking <b>ICM with EF \u2264 35%</b> or <b>Non-ICM with EF \u2264 35%</b> in Section 9.5a indications when you reach the Procedure step.’;
this.closest(’.sub-reveal’)?.appendChild(banner);
if(!this.closest(’.sub-reveal’)) this.parentNode.appendChild(banner);
}
}
} else {
document.getElementById(‘lvef_suggest’)?.remove();
}
});

// 8. Cardiac Arrest = Yes auto-suggest survivors indication
...
document.getElementById(‘arrest_suggest’)?.remove();
}
});
...
// 9. AFib = No + LAAO selected warning
...
// 10. Prior CIED = No default Generator to Initial implant
const ciedHidden = document.getElementById(‘val_cied’);
if(ciedHidden) {
new MutationObserver(() => {
if(ciedHidden.value === ‘No’) {
const initCrt = document.querySelector(’[name=“crtGenerator”][value=“Initial implant”]’);
const initPpm = document.querySelector(’[name=“ppmGenerator”][value=“Initial implant”]’);
if(initCrt && !document.querySelector(’[name=“crtGenerator”]:checked’)) {
initCrt.checked = true; onCrtGenChange();
}
if(initPpm && !document.querySelector(’[name=“ppmGenerator”]:checked’)) {
initPpm.checked = true; onPpmGenChange();
}
}
...
dupSnap.forEach(docSnap => {
    if(docSnap.id !== recordId && docSnap.data().civilId === civilIdVal) dupFound = true;
});
if(dupFound) {
    if(!confirm(`\u26a0 A record with Civil ID ${civilIdVal} already exists in the database.\n\nSave as a new record anyway?`)) return;
}
...
tr.innerHTML = ` <td><div class="font-bold text-gray-800">${fullName||'Anonymous'}</div><div class="text-xs text-gray-500">${data.gender||''} | Age: ${data.estimatedAge||'?'}</div></td> <td class="text-xs"><div><span class="font-semibold text-gray-500">CID:</span> ${data.civilId||'\u2014'}</div><div><span class="font-semibold text-gray-500">File:</span> ${data.fileNumber||'\u2014'}</div></td> <td><span class="badge badge-blue">${data.epReason||'\u2014'}</span></td> <td class="text-xs text-gray-600">${data.admissionDate||'\u2014'}</td> <td class="text-right space-x-1"> <button onclick='editRecord(${JSON.stringify(data).replace(/'/g,"\\'")} )' class="btn btn-yellow text-xs px-2 py-1"><i class="fas fa-edit"></i></button> <button onclick="deleteRecord('${data.id}')" class="btn btn-red text-xs px-2 py-1"><i class="fas fa-trash"></i></button> </td>`;
