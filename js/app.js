...
// (Skip identification removed — fields are now mandatory)
...
// Range validator — attaches to a numeric input
...
// LVEF — special: warn if unusual, confirm if <10 or >80
...
document.querySelector('[name="procEndTime"]')?.addEventListener('change', function() {
    ...
    const date  = document.querySelector('[name="procStartDate"]')?.value;
    const disch = document.querySelector('[name="procStartDate"]')?.value;
...
// 5. Civil ID century digit validation (must start with 2 or 3)
...
lvefInput?.addEventListener('blur', function() {
    ...
    if(!isNaN(v) && v <= 35) {
        ...
        const hxIcm = document.getElementById('val_icm')?.value === 'Yes';
...
// 8. Cardiac Arrest = Yes → auto-suggest survivors indication in 9.5a
document.getElementById('val_cardArrest')?.addEventListener('change', function() {});
// Watch via setYN — handled by observer pattern on the hidden field
